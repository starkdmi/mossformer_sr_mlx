...
func processAudio(
    ...
    if Float(inputLen) > Float(samplingRate) * oneTimeDecodeLength {
        ...
        _ = MLX.min(melSpec).item(Float.self)
        _ = MLX.max(melSpec).item(Float.self)
        _ = MLX.mean(melSpec).item(Float.self)
        _ = std(melSpec).item(Float.self)
        ...
        _ = MLX.min(outputs).item(Float.self)
        _ = MLX.max(outputs).item(Float.self)
        _ = MLX.mean(MLX.abs(outputs)).item(Float.self)
        _ = std(outputs).item(Float.self)
    ...
    do {
        ...
        print("Successfully saved output to: \(outputPath), \(Float(outputs.shape[0])/48000)s at 48kHz")
    } catch {
...
struct Generate: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "MossFormer2 Super Resolution - Upsample audio from 16kHz to 48kHz",
        discussion: """
        Upsamples audio to 48kHz using MossFormer2 SR model.
        Automatically downloads the model from HuggingFace on first run.

        If --output is not specified, saves as 'superres_48k.wav' in the input directory.
        """
    )

    @Option(name: .shortAndLong, help: "Path to input audio file")
    var input: String

    @Option(name: .shortAndLong, help: "Path to output audio file (default: superres_48k.wav in input directory)")
    var output: String?
    ...
    func run() throws {
        printSeparator()
        print("MossFormer2 Super Resolution - MLX Swift")
        printSeparator()
        ...
        if let output = output {
            ...
            outputPath = inputURL.deletingLastPathComponent()
                .appendingPathComponent("superres_48k.wav").path
        }
        ...
        print("Weight loading complete in \(String(format: "%.3f", Date().timeIntervalSince(loadStart)))s, total weights: \(filteredWeights.count)")
        ...
        let totalStart = Date()
        _ = try processAudio(model: model, audioPath: input, outputPath: outputPath, args: args)
